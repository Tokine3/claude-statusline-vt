...
format_epoch_time() {
  ...
  local result
  # macOS: date -j -f "%s", Linux: date -d "@epoch"
  result=$(TZ="Asia/Tokyo" date -j -f "%s" "$epoch" "$format" 2>/dev/null || \
           TZ="Asia/Tokyo" date -d "@${epoch}" "$format" 2>/dev/null || echo "")
  # %l に空白が入る場合を除去、AM/PM を小文字化
  echo "$result" | sed 's/^ //;s/ AM/am/;s/ PM/pm/;s/AM/am/;s/PM/pm/'
}

five_reset_display=""
if [ -n "$FIVE_HOUR_RESET" ] && [ "$FIVE_HOUR_RESET" != "0" ]; then
  five_reset_display="Resets $(format_epoch_time "$FIVE_HOUR_RESET" "+%l%p") (Asia/Tokyo)"
fi

seven_reset_display=""
if [ -n "$SEVEN_DAY_RESET" ] && [ "$SEVEN_DAY_RESET" != "0" ]; then
  seven_reset_display="Resets $(format_epoch_time "$SEVEN_DAY_RESET" "+%b %e at %l%p") (Asia/Tokyo)"
fi
...
if is_bonus_time; then
  sec=$(date +%S)
  offset=$(( 10#$sec % 7 ))
  bonus_line="$(rainbow_text "⚡ BONUS TIME!! ⚡" "$offset")"
  printf '%s\n' "$bonus_line"
fi
printf '%s\n' "$line1"
printf '%s\n' "$line2"
